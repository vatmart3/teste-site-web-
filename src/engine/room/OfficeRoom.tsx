...
const RANK: Record<OfficeKind, number> = {
  "office-intern": 0,
  "office-associate": 1,
  "office-senior": 2,
  "office-partner": 3,
...
function Shell({ rank }: { rank: number }) {
  ...
  const wall = useMat(
    () => PBR.plaster({ color: wallColor, repeat: [3, 2] }),
    [wallColor],
  );
  const floor = useMat(
    () =>
      rank >= 2
        ? PBR.wood(rank === 3 ? "walnut" : "oak", { repeat: [3, 3] })
        : PBR.carpet({ color: rank === 0 ? "#3a3d42" : "#2a3040" }),
    [rank],
  );
  const ceiling = useMat(
    () => PBR.plaster({ color: "#d8d6d0", repeat: [3, 3] }),
    [],
  );
  const base = useMat(
    () =>
      rank >= 2
        ? PBR.wood("walnut", { repeat: [4, 0.2] })
        : new THREE.MeshStandardMaterial({ color: "#2a2a2c", roughness: 0.6 }),
    [rank],
  );
  ...
  return (
    <group>
      <mesh
        position={[0, ROOM.floorY, ROOM.backZ + D / 2]}
      ...
      <mesh
        position={[0, ROOM.ceilingY, ROOM.backZ + D / 2]}
        rotation={[Math.PI / 2, 0, 0]}
        material={ceiling}
      >
        <planeGeometry args={[W, D]} />
      ...
      <mesh
        position={[-ROOM.sideX, midY, ROOM.backZ + D / 2]}
      ...
      {/* Plinthe */}
      <mesh
        position={[0, ROOM.floorY + 0.06, ROOM.backZ + 0.01]}
        material={base}
      >
    ...
    <group position={[0, ROOM.floorY + h / 2, ROOM.backZ + 0.012]}>
      ...
      {Array.from({ length: panels }, (_, i) => (
        <mesh
          key={i}
          material={wood}
          position={[-W / 2 + (i + 0.5) * (W / panels), 0.02, 0.014]}
        >
...
function Desk({ rank }: { rank: number }) {
  ...
  const top = useMat(
    () =>
      PBR.wood(rank === 0 ? "laminate" : rank === 1 ? "oak" : "walnut", {
        repeat: [1, 1],
      }),
    [rank],
  );
  const geo = useMemo(
    () => new RoundedBoxGeometry(w, d.thickness, depth, 4, 0.012),
    [w, depth, d.thickness],
  );
  const blotter = useMat(
    () => PBR.leather({ color: "#1e2a24", repeat: [3, 2] }),
    [],
  );
  return (
    <group>
      <mesh
        geometry={geo}
        material={top}
        position={[(d.x0 + d.x1) / 2, d.y - d.thickness / 2, (d.z0 + d.z1) / 2]}
        receiveShadow
        castShadow
      />
      {rank >= 1 && (
...
function Bookshelf({ rank }: { rank: number }) {
  ...
  const wood = useMat(
    () => PBR.wood(rank === 0 ? "laminate" : "walnut", { repeat: [0.5, 2] }),
    [rank],
  );
  ...
  return (
    <group ref={ref}>
      ...
      <mesh
        material={wood}
        position={[(x0 + x1) / 2, ROOM.floorY + 1.05, z - 0.15]}
      >
      ...
      {rank === 3 && (
        <Decanter position={[x1 - 0.12, shelves[3]! + 0.0125, z]} />
      )}
    </group>
  );
}
...
const NOTES: {
  ...
  size: [number, number];
  rot: number;
  note: Parameters<typeof noteTexture>[0];
}[] = [
  {
    at: [-0.62, 0.3],
    size: [0.2, 0.2],
    ...
    note: {
      title: "Client",
      lines: ["Fonds de pension", "des chauffeurs", "−60 M$ / 18 mois"],
      kind: "paper",
    },
  },
  {
    ...
    size: [0.18, 0.18],
    rot: -0.06,
    note: { title: "Meridian HQ", lines: [], kind: "photo" },
  },
  {
    at: [-0.16, 0.05],
    size: [0.15, 0.15],
    rot: 0.08,
    note: { title: "Rendu", lines: ["Demain", "8 h 00"], kind: "yellow" },
  },
  {
    at: [-0.55, 0.02],
    size: [0.19, 0.16],
    ...
    note: {
      title: "Cible",
      lines: ["Meridian Logistics", "4 000 camions", "Nasdaq"],
      kind: "paper",
    },
  },
];
...
function Corkboard() {
  const ref = useRef<THREE.Group>(null);
  // Tableau d'enquête interactif ouvert : les notes décoratives laissent la place aux pièces (Board3D).
      ...
      NOTES.map(
        (n, i) =>
          new THREE.MeshStandardMaterial({
            map: noteTexture(n.note, String(i)),
            roughness: 0.9,
          }),
      ),
    [],
  ...
  const string = useMemo(() => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(-0.62, 0.39, 0.012),
      new THREE.Vector3(-0.45, 0.3, 0.018),
      new THREE.Vector3(-0.3, 0.34, 0.012),
    ]);
    const curve2 = new THREE.CatmullRomCurve3([
      new THREE.Vector3(-0.3, 0.34, 0.012),
      new THREE.Vector3(-0.22, 0.2, 0.018),
      new THREE.Vector3(-0.16, 0.12, 0.012),
    ]);
    return [
      new THREE.TubeGeometry(curve, 24, 0.0015, 6),
      new THREE.TubeGeometry(curve2, 24, 0.0015, 6),
    ];
  }, []);
  const red = useMat(
    () => new THREE.MeshStandardMaterial({ color: "#a01414", roughness: 0.7 }),
    [],
  );
  useShadowFlags(ref);
  return (
    <group ref={ref} position={[0, 0, ROOM.backZ + 0.02]}>
      <mesh material={frame} position={[-0.48, 0.16, 0]}>
        <boxGeometry args={[1.0, 0.66, 0.03]} />
      </mesh>
      <mesh material={cork} position={[-0.48, 0.16, 0.017]}>
        <boxGeometry args={[0.94, 0.6, 0.01]} />
      </mesh>
      {!boardActive &&
        NOTES.map((n, i) => (
          <group
            key={i}
            position={[n.at[0], n.at[1] - 0.08, 0.024]}
            rotation={[0, 0, n.rot]}
          >
...
function WallClock() {
  ...
  const face = useMat(
    () => new THREE.MeshStandardMaterial({ map: clockFace(), roughness: 0.6 }),
    [],
  );
  const rim = useMat(
    () =>
  ...
  const hand = useMat(
    () => new THREE.MeshStandardMaterial({ color: "#111", roughness: 0.4 }),
    [],
  );
  const redHand = useMat(
    () => new THREE.MeshStandardMaterial({ color: "#b01818", roughness: 0.4 }),
    [],
  );
  useFrame(() => {
    const t =
      roomClock.mode === "ny"
        ? newYorkTime()
        : clock(
            Math.floor(roomClock.minutes / 60) % 24,
            Math.floor(roomClock.minutes % 60),
            Math.floor((roomClock.minutes % 1) * 60),
          );
  ...
  return (
    <group position={[0.16, 0.34, ROOM.backZ + 0.03]}>
      ...
      <mesh ref={hour} material={hand} position={[0, 0, 0.004]}>
        <boxGeometry
          args={[0.012, 0.075, 0.003]}
          onUpdate={(g) => g.translate(0, 0.03, 0)}
        />
      </mesh>
      <mesh ref={minute} material={hand} position={[0, 0, 0.007]}>
        <boxGeometry
          args={[0.007, 0.115, 0.003]}
          onUpdate={(g) => g.translate(0, 0.05, 0)}
        />
      </mesh>
      <mesh ref={second} material={redHand} position={[0, 0, 0.01]}>
        <boxGeometry
          args={[0.003, 0.13, 0.002]}
          onUpdate={(g) => g.translate(0, 0.045, 0)}
        />
      </mesh>
      <mesh
        material={glass}
...
function Monitor({ keyboard }: { keyboard: boolean }) {
  ...
  const keys = useMat(
    () => new THREE.MeshStandardMaterial({ color: "#1b1c20", roughness: 0.55 }),
    [],
  );
  const geo = useMemo(
    () => ({
      bezel: new RoundedBoxGeometry(0.6, 0.36, 0.025, 3, 0.008),
      kb: new RoundedBoxGeometry(0.42, 0.018, 0.14, 3, 0.006),
    }),
    [],
  );
  useShadowFlags(ref);
    ...
    <group ref={ref}>
      ...
      {keyboard && (
        <mesh
          geometry={geo.kb}
          material={keys}
          position={[0.2, ROOM.desk.y + 0.009, -0.85]}
          rotation={[0, -0.04, 0]}
        />
      )}
    </group>
...
function Door() {
  ...
  const who = useMat(
    () => new THREE.MeshBasicMaterial({ map: silhouette(), transparent: true }),
    [],
  );
  ...
  return (
    <group ref={ref}>
      ...
      <mesh
        ref={passer}
        material={who}
        position={[x, y - 0.05, ROOM.backZ - 0.35]}
        userData={{ noShadow: true }}
      >
...
function Window({ rank, variant }: { rank: number; variant: Variant }) {
  const view = useMat(
    () =>
      new THREE.MeshBasicMaterial({ map: cityView(variant), toneMapped: true }),
    [variant],
  );
  const frame = useMat(
    () =>
    ...
    <group ref={ref} position={LAMP_POS}>
      ...
      <group position={[0, 0.39, 0.06]} rotation={[0, 0, Math.PI / 2]}>
        <mesh material={shade} rotation={[0, 0, 0]}>
          <cylinderGeometry
            args={[0.075, 0.075, 0.34, 48, 1, true, Math.PI, Math.PI]}
          />
        </mesh>
        <mesh>
          <cylinderGeometry
            args={[0.073, 0.073, 0.335, 48, 1, true, 0, Math.PI]}
          />
...
function PlasticPlant() {
  const ref = useRef<THREE.Group>(null);
  const pot = useMat(
    () => new THREE.MeshStandardMaterial({ color: "#d9d4ca", roughness: 0.35 }),
    [],
  );
  const leafMat = useMat(
    () =>
      new THREE.MeshPhysicalMaterial({
        color: "#3f7a3a",
        roughness: 0.3,
...
function Frames({ rank }: { rank: number }) {
  ...
  const dip = useMat(
    () => new THREE.MeshStandardMaterial({ map: diploma(), roughness: 0.8 }),
    [],
  );
  const art = useMat(
    () => new THREE.MeshStandardMaterial({ map: painting(), roughness: 0.9 }),
    [],
  );
  const items: {
    pos: [number, number];
  ...
  if (rank >= 2)
    items.push({ pos: [0.95, 0.62], size: [0.26, 0.19], mat: dip });
  return (
...
function RoomLights({ rank, variant }: { rank: number; variant: Variant }) {
  ...
  useFrame(({ clock }) => {
    ...
    const cut =
      rank === 0 &&
      !rig.reducedMotion &&
      Math.sin(t * 13.7) * Math.sin(t * 3.1) > 0.93
        ? 0.35
        : 1;
    const k = roomState.neon * cut;
    if (neon.current)
      neon.current.intensity =
        (rank === 0 ? 5.5 : 2.2) * k * (variant === "day" ? 0.4 : 1);
  ...
  return (
    <group>
      <rectAreaLight
        ref={neon}
        position={[0, ROOM.ceilingY - 0.02, -1.0]}
        rotation={[-Math.PI / 2, 0, 0]}
        width={1.2}
        height={0.18}
        color={rank === 0 ? "#dfe9ff" : "#fff1dc"}
        intensity={4}
      />
      <mesh
        position={[0, ROOM.ceilingY - 0.015, -1.0]}
        rotation={[Math.PI / 2, 0, 0]}
        userData={{ noShadow: true }}
      >
      ...
      <hemisphereLight
        args={["#fff2e0", "#2a2018", variant === "night" ? 0.12 : 0.35]}
      />
    </group>
  );
}
...
export function OfficeRoom({
  ...
  return (
    <group>
      <SceneEnvironment
        name="apartment"
        intensity={variant === "night" ? 0.16 : variant === "dusk" ? 0.3 : 0.5}
      />
      <Shell rank={rank} />
      <Desk rank={rank} />
      <Bookshelf rank={rank} />
...
export const ROOM_HOTSPOTS = {
  library: {
    position: [-1.4, ROOM.floorY + 1.2, ROOM.backZ + 0.2] as [
      number,
      number,
      number,
    ],
    size: [0.8, 1.6, 0.3] as [number, number, number],
  },
  board: {
    position: [-0.48, 0.16, ROOM.backZ + 0.04] as [number, number, number],
    size: [1.0, 0.66, 0.04] as [number, number, number],
  },
  terminal: {
    position: [MONITOR_POS[0], MONITOR_POS[1] + 0.34, MONITOR_POS[2]] as [
      number,
      number,
      number,
    ],
    size: [0.6, 0.36, 0.03] as [number, number, number],
  },
  door: {
    position: [1.35, ROOM.floorY + 1.06, ROOM.backZ + 0.03] as [
      number,
      number,
      number,
    ],
    size: [0.95, 2.15, 0.06] as [number, number, number],
  },
} as const;
